...
form.addEventListener("submit", (e) => {
  ...
  let astrologyCard = document.getElementById("astrologyCard");
  const myImage = document.getElementById("myImage");
  const formContainer = document.getElementById("form");

  let index = (firstName.length * surname.length) % 20;

  const fristMessage = zodiacSigns[month - 1];
  const secondMessage = compliments[date];
  const thirdMessage = victimCardCompliments[index];

  index = (gender.length * pob.length) % 30;
  const fourtMessage = recommendations[index];

  index = (firstName.length * pob.length) % 20;
  const fifthMessage = predictions[index];

  myImage.style.display = "none";
  formContainer.style.display = "none";
  astrologyCard.style.display = "block";

  astrologyCard.innerText = `Hi ${firstName} ${surname}. Your Zodiac sign is ${fristMessage} ${secondMessage} ${thirdMessage} My recommendation is ${fourtMessage} and my predictions about you: ${fifthMessage}`;
});
